...
The inbox is the server-held buffer that stands in for a push channel. Its contract is exactly the core's relay floor (Core §4.3, Delivery §6), realized as storage rather than delivery:
...
- **Drain-on-read, at-most-once.** `Receive` removes what it returns. The profile does not define acknowledgements, cursors, or redelivery; a payload returned by `Receive` but lost before the client uses it is gone. Reliability beyond this floor is out of scope and is deferred to a future document (Delivery §7).
