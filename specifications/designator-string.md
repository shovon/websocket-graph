...
This document uses the terms **Server**, **Client**, **Node**, **out-neighbor**, **neighborhood**, **neighborhood state**, and **Designator** as defined in Architecture §2 and refined in Delivery §2. A **designator string** is the concrete realization of a `Designator` this binding fixes.
...
- The maximum length of a designator string is not fixed here; it is bounded by the transport binding or deployment (Delivery §7). A server SHOULD keep designators short, as they are server-generated and a client only ever echoes them back (Section 3).
...
1. **Select** a designator verbatim from its *current* neighborhood state (Delivery §5), to name the out-neighbor that state associates with it.
...
This binding fixes no meaning for a designator beyond the single neighborhood state in which it appears, and likewise fixes no relationship between designators drawn from different states. Whether designators may be compared across states, and what such a comparison would mean, is not defined here; a client that needs continuity of identity across neighborhood changes constructs it above this interface (Architecture §3.2), and a future layer MAY give designators cross-state meaning (Delivery §2).

## 4. Distinct Designation

Within a node's **single current neighborhood state**, the server MUST assign **pairwise-distinct** designator strings to that node's out-neighbors — no two out-neighbors of the same node share a designator in the same state — so that each out-neighbor can be addressed unambiguously (Delivery §2).
...
The distinctness requirement is the *only* uniqueness this binding mandates. Consistent with Delivery §2, a server is **not** required to make a designator globally unique, persistent across reconnection, or stable across a node's neighborhood changes. A server MAY reuse a string to denote a different out-neighbor in a later state, and MAY change the string that denotes a given out-neighbor from one state to the next.

An implementation MAY provide guarantees stronger than per-state distinctness — stability across states, global uniqueness, unguessability (Section 7), or persistence across a node's session boundary, the last where a derived identity model supplies a durable node identity (Delivery §2, Architecture §3) — and, if it does, SHOULD document them. Absent such a documented guarantee, this binding provides none beyond per-state distinctness, and what may be built on a stronger guarantee is a matter for the layer that defines it (Delivery §2).
...
Because matching is exact equality against the current neighborhood, a designator that does not denote a present out-neighbor yields discard, never misdelivery and never reach to a non-neighbor (Delivery §3, §4). Per-state distinctness (Section 4) guarantees the match is unambiguous: at most one current out-neighbor can satisfy it.
