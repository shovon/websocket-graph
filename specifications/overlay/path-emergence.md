...
This document is **non-normative**. Like the base architecture (`architecture.md`), it is aspirational: it sketches the shape of an algorithm and argues for that shape, but it fixes no wire format, mandates no field, and defines no conformance surface. It is the algorithm companion to the architecture, which describes the *substrate* clients build on. This memo is what that substrate was pointing at — but only its **first move**: how a single flood **discovers a trail** to a distant node. How that trail is then proven, reused, and replaced when it rots is the work of its companion, `path-completion.md`; carrying it to the party who needs it is only the same rule run again (`sending-rule.md`). This memo deliberately stops at discovery.

Read it for the *why* and the *how-in-outline*. When a future normative companion fixes the packet formats, the loop-detection rules, and the timeouts, that document carries the requirements; this one carries the reasoning they were derived from.
...
This memo describes the first half of that emergence: **discovery**. A single flood, launched toward a target, lays down a record of how it travelled, and that record — a **trail** — comes to rest *at the target* as a reusable route. That is the whole of what is built here. The scheme is, in essence, the route-discovery phase of Dynamic Source Routing, adapted to a network whose edges do not run both ways: because no edge runs backward, the substrate hands a node nothing about where a message came from, so the trail cannot be read off after the fact — it must be written down explicitly, hop by hop, as the flood runs. What becomes of a trail once it exists — the proof that it holds when walked as a single line, and the reuse and repair that follow — is deferred to `path-completion.md`; carrying it from the target, where it comes to rest, into the hands of the party who will send along it is the same flooding rule run once more (`sending-rule.md`). Here, one flood lays one trail.

## Table of Contents

1. The problem, stated as a cost
2. What we get to assume
3. The flood that lays a trail
4. What is discovered, and what is deferred
...
Let $A$ want to reach a distant node $B$, and let $A$ hold no trail to it. $A$ has only the one move the substrate offers — send to an out-neighbor — and $B$ is not one. So $A$ falls back on flooding, and a trail assembles itself as the flood runs. Write the trail it will produce as $P_{AB}$.
...
Each node that receives a copy appends to the trail a pair *(my ID, the designator I am about to forward along)*, forwards a copy onward to each of its own out-neighbors, and asks: *am I $B$?* That a flood *terminates* — fans out, reaches every node, then dies rather than circulating forever — is something this memo leans on throughout; *how* it terminates, the loop-detection rule by which a node drops a copy instead of forwarding it, is for the normative layer to pin down. Assume here only that it is settled — but settled toward the right target, because "terminates" hides a stronger requirement than it first shows. The easy reading is *finiteness*: no single copy circulates forever, because every copy eventually meets a node that drops it. That much is nearly automatic, and it is all a flood on a cyclic graph needs in order to stop being a storm. But the $O(E)$ price quoted in §1 quietly assumes *more* — not merely that each copy dies, but that copies do not *pile up*: that each node forwards each flood about once, so the cost is one send per edge and not one per path that reaches the node (and the number of paths is exponential). A rule can buy finiteness and still leave that second property wide open, passing copies that each die promptly yet multiply on the way down. So the loop-detection rule must secure *both*, and whoever weighs such a rule should know it is buying two things and not one. The amortization §1 promises rests on the second.

When the node that is $B$ asks that question and the answer is yes, the copy it holds carries, in its accumulated list, a complete record of how a message travelled from $A$ to here — every hop's ID and the designator each used. **That record is $P_{AB}$, the trail, and it now exists at $B$.**

Note carefully *where* it exists, because it is the whole reason there is a companion memo. It exists **only** at $B$. $A$ does not have it. $A$ launched the flood but has no idea which branch reached $B$, or by what path, or even that it arrived at all. The trail was discovered at the far end from the party who will use it — the most useful thing the flood produced now sits in the hands of the one node that has no need of it.

And nothing here is yet *proven*. $P_{AB}$ was assembled under flooding, where copies fan out in every direction at once; it is a *candidate* — a record of how a flood once travelled, not a route yet shown to work when walked as a single directed line. Promoting a candidate to a proven trail belongs to completion (`path-completion.md`), not to discovery; carrying it home to $A$ is just the rule run again (`sending-rule.md`).
...
**What is deferred.** Everything that turns that residue into a working conversation:

- *The trail is at the wrong end.* $P_{AB}$ sits at $B$, but $A$ is the one who will send along it. Getting it into $A$'s hands is not a new mechanism but the same move run again — the rule, applied by $B$, carrying cargo (`sending-rule.md`).
- *The other direction is this same trick again.* Should $B$ ever need to reach $A$, the directed graph (§2) gives it no way to answer along the trail just laid — but it needs no new machinery: it floods in its turn, and a trail to $A$ assembles itself by this very construction with the names swapped. There is nothing further to discover there, and so nothing further said of it here; the reader who wants it has already seen how it goes.
- *Nothing is proven.* A candidate trail is not yet a working one; promoting it to a route shown to hold when walked as a single directed line — and deciding what to do when that fails — is completion's.
...
The rest — giving the granted unique ID real teeth as a key, and admitting the world was never perfect — is deferred work, the hardening that comes once the perfect world ends, and not this memo's. Here, one flood lays one trail, and a path begins to emerge.
