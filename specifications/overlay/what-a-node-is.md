...
Section references resolve by name, not by location. Those of the form (Emergence §N) point into `path-emergence.md`, (Completion §N) into `path-completion.md`, (Sending §N) into `sending-rule.md`, and (Hardening §N) into `hardening-the-overlay.md` — companions in this same folder. (Architecture §N) points into `../architecture.md` and (Payload §N) into `../payload-string.md` — substrate documents one level up.
